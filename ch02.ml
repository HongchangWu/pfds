  ...
  *)
  let insert x s =
    let rec go z = function
      | E ->
        (match z with
        | Some y when x = y -> failwith "Found"
        | _ -> T (E, x, E))
      | T (a, y, b) -> if Element.lt x y then T (go z a, y, b) else T (a, y, go (Some y) b)
    in
    try go None s with
    | Failure _ -> s
  ;;
...
module UnbalancedMap (Key : ORDERED) : FiniteMap with type key = Key.t = struct
  ...
  let rec bind = function
    ...
    | k, x, T (a, (k', y), b) ->
      if Key.lt k k'
      then T (bind (k, x, a), (k', y), b)
      else if Key.lt k' k
      then T (a, (k', y), bind (k, x, b))
      else T (a, (k, x), b)
  ;;
  ...
  let rec lookup = function
    | _, E -> raise Not_found
    | k, T (a, (k', y), b) ->
      if Key.lt k k' then lookup (k, a) else if Key.lt k' k then lookup (k, b) else y
  ;;
end
